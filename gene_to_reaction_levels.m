function reaction_levels = gene_to_reaction_levels( model, genes, levels)
...
%       reaction_levels - reaction expression levels
    ...
    for i = 1:length(model.rxns)
        level = eval_gpr(model.grRules{i}, genes, levels);
        reaction_levels(i) = level;
    end

end

function [result, status] = eval_gpr(rule, genes, levels)
    f_and=@min;
    f_or=@(x,y)(x+y);
    EVAL_OK = 1;
